...
- Current execution brief:
  `T-049` final iOS release-candidate polish and baseline lock
- Current execution task:
  treat `T-035`, `T-036`, and `T-037` as the accepted hardened rollout baseline, treat `T-038` as the accepted OTA safety baseline, treat `T-039` as the accepted backend control-plane baseline, treat `T-040` as the accepted firmware OTA client baseline, treat `T-041` as the accepted real OTA validation baseline on immutable release `fw-beta-20260327-05`, treat `T-042` as the accepted minimum owner-facing update surface, treat `T-043` as the accepted operator rollout/rollback baseline, treat `T-044` as the accepted blocker audit baseline, treat `T-045` as the closed external blocker-remediation checkpoint, treat `T-051` as the accepted broker credential-sync reliability fix, and treat `T-053` as the accepted Meditation recovery support slice; then run `T-049` to finalize sign-in/auth copy, align Expo SDK 55 patch dependencies, and fold the accepted `T-046`, `T-047`, and `T-048` app fixes into one explicit ship baseline; then run `T-050` as the last major product-facing polish slice for first-device onboarding and setup before the next iOS RC build and store-submission prep, while keeping Android on a follow-up track unless it is intentionally pulled back into the same launch wave. `T-052` remains preserved as a later proof slice for ownership-transfer and backup-retention validation, but it is not part of the current publish-critical path.
- Parallel exception:
  ...
  `T-048` is now accepted as a narrow Home confirmation-latency follow-up on top of `T-047`; the main sync path is materially better, stale reload truth is fixed, and the unplug-and-pull-to-refresh proof is complete
- Git durability note:
