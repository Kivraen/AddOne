...
- Current execution task:
  treat `T-035`, `T-036`, and `T-037` as the accepted hardened rollout baseline, treat `T-038` as the accepted OTA safety baseline, treat `T-039` as the accepted backend control-plane baseline, treat `T-040` as the accepted firmware OTA client baseline, then finish `T-041` by capturing one clean real-hardware confirmation pass for the active beta.3 OTA release
- Git durability note:
