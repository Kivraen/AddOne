...
- Parallel exception:
  `T-046` may run as a narrow UI-only cleanup slice while `T-045` is blocked, but weekly-minimum semantics and offline-sync reliability must stay in a later planning-oriented follow-up instead of being mixed into that first cleanup pass
- Git durability note:
