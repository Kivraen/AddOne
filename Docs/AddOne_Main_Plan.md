...
- Current execution task:
  treat `T-035`, `T-036`, and `T-037` as the accepted hardened rollout baseline, treat `T-038` as the accepted OTA safety baseline, treat `T-039` as the accepted backend control-plane baseline, treat `T-040` as the accepted firmware OTA client baseline, treat `T-041` as the accepted real OTA validation baseline on immutable release `fw-beta-20260327-05`, treat `T-042` as the accepted minimum owner-facing update surface, treat `T-043` as the accepted operator rollout/rollback baseline, treat `T-044` as the accepted blocker audit baseline, treat `T-045` as the closed external blocker-remediation checkpoint, and treat `T-051` as the accepted broker credential-sync reliability fix; then run `T-049` to finalize sign-in/auth copy, align Expo SDK 55 patch dependencies, and fold the accepted `T-046`, `T-047`, and `T-048` app fixes into one explicit ship baseline; then run `T-050` as the last major product-facing polish slice for first-device onboarding and setup before the next iOS RC build and store-submission prep, while keeping Android on a follow-up track unless it is intentionally pulled back into the same launch wave. In parallel, `T-052` is now available as a narrow validation slice to prove that ownership transfer revokes stale viewer access and preserves prior-owner backups privately instead of carrying that path as an assumption.
- Parallel exception:
