...
`T-034` is now accepted as the planning checkpoint, and `T-035` is now accepted as the first code-complete hardening baseline on `codex/s4-transport-trust-and-device-identity`. `T-036` now has a real hosted checkpoint on `codex/s4-release-operations-baseline`, but it remains `revise and retry` because the hardened device still fails MQTT TLS reconnect and the broker still logs `ssl/tls alert bad certificate`. The next slice is [T-037-mqtt-tls-acceptance-and-device-reprovisioning.md](/Users/viktor/Desktop/DevProjects/Codex/AddOne/Docs/tasks/T-037-mqtt-tls-acceptance-and-device-reprovisioning.md): fix the ESP32 broker TLS acceptance and get both beta devices onto the per-device credential path before OTA safety or rollout tooling proceeds.
