...
`T-034` is now accepted as the planning checkpoint, `T-035` is now accepted as the first code-complete hardening baseline on `codex/s4-transport-trust-and-device-identity`, both `T-036` plus `T-037` are now accepted on `codex/s4-release-operations-baseline`, `T-038` is now accepted on `codex/s4-firmware-ota-safety`, and `T-039` is now accepted on `codex/s4-firmware-ota-control-plane`. The hosted rollout baseline is real end to end, the OTA safety contract is explicit in-repo, and the backend OTA control plane now enforces paused or rolled-back releases correctly during in-progress re-checks. The next narrow slice is `T-040`: implement the firmware OTA client against those accepted contracts.
