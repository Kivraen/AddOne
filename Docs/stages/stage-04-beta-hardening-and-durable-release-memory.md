...
`T-034` is now accepted as the planning checkpoint, `T-035` is now accepted as the first code-complete hardening baseline on `codex/s4-transport-trust-and-device-identity`, both `T-036` plus `T-037` are now accepted on `codex/s4-release-operations-baseline`, `T-038` is now accepted on `codex/s4-firmware-ota-safety`, `T-039` is now accepted on `codex/s4-firmware-ota-control-plane`, and `T-040` is now accepted on `codex/s4-firmware-ota-client`. `T-041` is no longer blocked on hosted backend drift: the OTA schema is live remotely, the immutable artifact and release rows are real, the original `addone_sync` crash loop is fixed, and the earlier "applied command but no first OTA progress" gap is fixed. The remaining proof gap is now the artifact stream itself: the bench board reaches backend-visible `requested -> downloading`, then still fails before `downloaded`, with the latest no-serial retry stalling at `379900/1134144` bytes before any provisional boot or backend-visible `pending_confirm -> succeeded`.
