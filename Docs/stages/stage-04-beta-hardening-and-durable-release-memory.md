...
`T-034` is now accepted as the planning checkpoint, `T-035` is now accepted as the first code-complete hardening baseline on `codex/s4-transport-trust-and-device-identity`, both `T-036` plus `T-037` are now accepted on `codex/s4-release-operations-baseline`, and `T-038` is now accepted on `codex/s4-firmware-ota-safety`. The hosted rollout baseline is real end to end, and the OTA safety contract is now explicit in-repo. The next slice is [T-039-firmware-ota-control-plane-and-release-registry.md](/Users/viktor/Desktop/DevProjects/Codex/AddOne/Docs/tasks/T-039-firmware-ota-control-plane-and-release-registry.md): implement the backend release registry, eligibility RPCs, OTA progress reporting, and trigger path against the locked `T-038` contract.
