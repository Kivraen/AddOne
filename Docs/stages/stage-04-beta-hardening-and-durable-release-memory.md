...
`T-034` is now accepted as the planning checkpoint, `T-035` is now accepted as the first code-complete hardening baseline on `codex/s4-transport-trust-and-device-identity`, both `T-036` plus `T-037` are now accepted on `codex/s4-release-operations-baseline`, `T-038` is now accepted on `codex/s4-firmware-ota-safety`, `T-039` is now accepted on `codex/s4-firmware-ota-control-plane`, `T-040` is now accepted on `codex/s4-firmware-ota-client`, and `T-041` is now accepted on `codex/s4-firmware-ota-validation`. The immutable OTA path is now hardware-proven end to end, with authoritative successful proof on replacement release `fw-beta-20260327-05`, backend-visible `pending_confirm -> succeeded`, and stable post-window runtime on `2.0.0-beta.3`. The next slice should add the minimum app update and firmware status surfaces before broader rollout and rollback tooling.
