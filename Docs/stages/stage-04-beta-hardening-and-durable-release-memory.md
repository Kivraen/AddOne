...
`T-034` is now accepted as the planning checkpoint, `T-035` is now accepted as the first code-complete hardening baseline on `codex/s4-transport-trust-and-device-identity`, and both `T-036` plus `T-037` are now accepted on `codex/s4-release-operations-baseline`. The hosted rollout baseline is finally real end to end: migration applied, password rollout fixed, both beta boards on per-device MQTT credentials, and MQTT cut over to `mqtt-beta.addone.studio`. The next slice is [T-038-firmware-ota-safety-model-and-release-contract.md](/Users/viktor/Desktop/DevProjects/Codex/AddOne/Docs/tasks/T-038-firmware-ota-safety-model-and-release-contract.md): lock the OTA safety model before building the control plane or firmware client.
