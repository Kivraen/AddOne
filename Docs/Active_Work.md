...
- Next brief: [B-014-stage-s3-add-device-entry-flow-first-screen.md](briefs/B-014-stage-s3-add-device-entry-flow-first-screen.md)
...
| App beta surface | Implemented | The product shell now has the friend-facing profile model and the first Friends sharing implementation in place. Friends final acceptance is blocked on second-device proof, so onboarding and recovery polish are the current active slice. | Coordinator | `S3` remains active until Friends proof, the timezone revision path, and the final onboarding polish slice are all explicit enough to accept. |
...
| T-008 | Onboarding and Wi-Fi recovery polish batch | app | In Progress | Unassigned | T-001 | [T-008](tasks/T-008-onboarding-and-wifi-recovery-polish.md) | [Report](agent-reports/2026-03-19-s3-onboarding-and-wifi-recovery-polish.md) | Strict gate | Treat this as the parent slice for the current add-device and onboarding work. |
| T-017 | Add-device entry flow first-screen polish | app | Brief Ready | Unassigned | T-008 | [T-017](tasks/T-017-add-device-entry-flow-first-screen.md) | — | Strict gate | Start with [B-014](briefs/B-014-stage-s3-add-device-entry-flow-first-screen.md) to refine the no-owned-device Home screen before broadening the rest of onboarding. |
