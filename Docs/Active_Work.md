...
| App beta surface | Stale | The product shell exists, habit identity/minimum-goal UI has landed, and the home header now has a safer checking state, but `Friends` is still placeholder UI and several visible surfaces still need a coordinated polish and proof pass. | Coordinator | `S3` is active because the user reprioritized UI completion and social-shape lock. |
...
| T-005 | Beta UI audit and scope lock | app | In Progress | Codex | — | [T-005](tasks/T-005-beta-ui-audit-and-scope-lock.md) | [Report](agent-reports/2026-03-18-home-connection-checking-state.md) | Strict gate | Home connection-state slice and habit identity slice are implemented but not yet accepted; complete quick manual UI proof passes, then keep splitting profile, onboarding/recovery, and friends work. |
