...
- Next brief: [B-009-stage-s3-friends-plan-and-model-lock.md](briefs/B-009-stage-s3-friends-plan-and-model-lock.md)
...
| App beta surface | Stale | The product shell has a substantial S3 polish pass behind it now, but `Friends` is still placeholder UI and is now the next active planning focus. Onboarding is being intentionally held as the final visible UI polish slice after the friends checkpoint. | Coordinator | `S3` remains active until friends and onboarding are both explicit enough to accept. |
...
| T-005 | Beta UI audit and scope lock | app | In Progress | Codex | — | [T-005](tasks/T-005-beta-ui-audit-and-scope-lock.md) | [Report](agent-reports/2026-03-19-s3-cumulative-ui-surface-report.md) | Strict gate | The cumulative checkpoint is accepted, repo-wide typecheck signal is restored, and friends planning is now promoted ahead of final onboarding polish. |
| T-015 | Friends beta plan and model lock | product | Brief Ready | Unassigned | T-005 | [T-015](tasks/T-015-friends-beta-plan-and-model-lock.md) | — | Strict gate | Start with [B-009](briefs/B-009-stage-s3-friends-plan-and-model-lock.md) and turn the current friends ideas into a durable first-beta plan before implementation. |
