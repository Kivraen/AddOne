...
- Next brief: [B-017-stage-s3-friends-finalization-plan.md](briefs/B-017-stage-s3-friends-finalization-plan.md)
...
| App beta surface | Implemented | The product shell now has the friend-facing profile model, the first Friends sharing implementation, a hardware-validated setup or recovery flow, `Reset history` as `Start new habit`, and a live-validated destructive factory reset plus fresh re-add path. | Coordinator | `S3` remains active until Friends proof, the timezone revision path, and the remaining polish-focused onboarding or recovery slices are explicit enough to accept. |
...
| T-001 | Beta Friends surface and social floor | app | Blocked | Unassigned | T-015, T-009 | [T-001](tasks/T-001-beta-friends-surface.md) | [Report](agent-reports/2026-03-16-app-beta-friends-surface.md) | Strict gate | Use [T-020](tasks/T-020-friends-finalization-plan-and-proof-strategy.md) and [B-017](briefs/B-017-stage-s3-friends-finalization-plan.md) first to map the exact final proof path, then return to [B-012](briefs/B-012-stage-s3-beta-friends-verification-pass.md) for the closing verification sweep. |
...
| T-020 | Friends finalization plan and proof strategy | app | Brief Ready | Unassigned | T-001 | [T-020](tasks/T-020-friends-finalization-plan-and-proof-strategy.md) | — | Strict gate | Start with [B-017](briefs/B-017-stage-s3-friends-finalization-plan.md) to map the current Friends implementation and the exact proof/test path before resuming execution. |
