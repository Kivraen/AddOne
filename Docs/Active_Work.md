...
| Timezone model | Stale | Timezone values flow through the app, backend, and firmware, but the user-facing model, selector UX, and firmware support are not universal yet. | Coordinator | Audit in `T-006` before promising device-wide timezone flexibility. |
...
| T-006 | Timezone model and universal flow audit | cross-platform | Assigned | Delegated agent | — | [T-006](tasks/T-006-timezone-model-and-universal-flow-audit.md) | — | Strict gate | Review the audit against the stage note, then decide whether timezone is UI-only work or a firmware-backed beta blocker. |
