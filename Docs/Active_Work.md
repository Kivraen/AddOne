...
| App beta surface | Implemented | The product shell now has the friend-facing profile model, a live-verified first Friends sharing flow, a hardware-validated setup or recovery flow, `Reset history` as `Start new habit`, and a live-validated destructive factory reset plus fresh re-add path. | Coordinator | `S3` remains active until the timezone revision path and the remaining polish-focused onboarding or recovery slices are explicit enough to accept. |
