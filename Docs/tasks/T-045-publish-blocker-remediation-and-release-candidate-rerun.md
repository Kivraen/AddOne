...
- Android has no finished build unless launch is explicitly iOS-only

## In Scope
- Reflash `AO_A4F00F767008` to the accepted OTA-capable baseline or explicitly remove it from the RC cohort
- Produce fresh installable artifacts from the accepted release-candidate baseline
- Produce Android artifacts too unless the user explicitly narrows launch to iOS-only
...
- Fresh installable artifacts must exist for the accepted release-candidate baseline
