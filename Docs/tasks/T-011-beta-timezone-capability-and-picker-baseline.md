...
## Objective
Implement a beta-safe timezone model for AddOne so users can keep the device on the phone's timezone by default, change it later when the device moves, and search for additional timezones without the app promising unsupported on-device behavior.
