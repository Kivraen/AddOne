...
- The March 30 Supabase `rls_disabled_in_public` alert was real: `public.board_habit_eras` had been created without `ENABLE ROW LEVEL SECURITY`, and anonymous REST requests could read rows. Migration `20260330172500_enable_rls_on_board_habit_eras.sql` is now applied remotely, and the same anonymous REST check now returns `[]` with `content-range: */0`.
- `T-052` is now a deferred narrow validation slice: the ownership-transfer path appears to revoke old owner and viewer access correctly while preserving prior-owner backups privately, and the user chose to keep that exact owner A plus viewer B plus new owner C proof as later follow-up rather than a current publish gate.
