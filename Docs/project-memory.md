...
- `T-053` is now accepted on `codex/s4-meditation-firmware-rebaseline`: `AO_A4F00F767008` (`Meditation`) was recovered from stale pre-OTA-capable `2.0.0-beta.1` plus stale persisted MQTT creds to `2.0.0-beta.3`, MQTT health returned, and live control was re-verified. The board remains intentionally outside the active ship cohort.
