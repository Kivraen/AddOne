...
alter table public.device_memberships
  ...
  check (celebration_dwell_seconds between 1 and 60);
...
begin
  ...
  if next_transition_speed is not null and next_transition_speed not in ('very_fast', 'fast', 'balanced', 'slow', 'even_slower') then
    raise exception 'Unsupported celebration speed.';
  end if;

  if next_dwell_seconds is not null and (next_dwell_seconds < 1 or next_dwell_seconds > 60) then
    raise exception 'Celebration dwell seconds must be between 1 and 60.';
  end if;
