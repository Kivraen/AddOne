...
declare
  ...
  result_update_available boolean := false;
  result_can_request_update boolean := false;
begin
          ...
          if candidate_release.minimum_confirmed_firmware_version is not null
            ...
            if normalized_app_version is null then
              ...
              if app_compare is null then
                ...
                result_update_available := not ota_in_progress;
                result_can_request_update := not ota_in_progress;
              end if;
            end if;
          else
            result_reason := case
  ...
  select
    ...
    result_can_request_update;
end;
$$;

revoke all on function public.get_device_firmware_update_summary(uuid, text) from public;

grant execute on function public.get_device_firmware_update_summary(uuid, text) to authenticated, service_role;
